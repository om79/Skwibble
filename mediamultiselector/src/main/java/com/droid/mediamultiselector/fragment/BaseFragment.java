        ...
        if (ContextCompat.checkSelfPermission(getContext(), Manifest.permission.WRITE_EXTERNAL_STORAGE)
            ...
            if (intent.resolveActivity(getActivity().getPackageManager()) != null) {
                ...
                if (mTmpFile != null && mTmpFile.exists()) {
                    intent.putExtra(MediaStore.EXTRA_OUTPUT, FileProvider.getUriForFile(getActivity(), getActivity().getApplicationContext().getPackageName() + ".provider", mTmpFile));
...
//        Intent intent=new Intent(Intent.ACTION_VIEW, Uri.parse(video.path));
//        intent.setClassName(getActivity(), VideoViewActivity.class.getName());
//        startActivity(intent);
        Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(video.path));
        intent.setDataAndType(Uri.parse(video.path), "video/mp4");
        startActivity(intent);
    }
}
